...
local function isAtCroesus()
    local CROESUS_AREA = { x = 1914, y = 1248, z = 0 }
    return utils.isAtLocation(CROESUS_AREA, 50)
end

local function shouldTeleWars()
    local isInWars = utils.isInWarsRetreat()
    if isInWars then
        return false
    end
    ...
    if not isInWars and not isAtCroesus() then
        return true
    end
...
local function teleCroesus()
    API.DoAction_Object_string1(0x39, 0, { "Portal (Croesus)" }, 50, true)
